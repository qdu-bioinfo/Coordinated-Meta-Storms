...
int Check_Format(const char *infilename) {
    ...
    if ((label[0] == '>') &&
        ((seq[0] == 'a') || (seq[0] == 't') || (seq[0] == 'c') || (seq[0] == 'g') || (seq[0] == 'A') || (seq[0] == 'T') || (seq[0] == 'C') ||
         (seq[0] == 'G') || (seq[0] == 'N') || (seq[0] == 'n')))
...
void Make_list(const char *listname, const char *outpathname, vector <string> ids, int mode) { //0: classification.txt; 1: functions.txt; 2: id
    ...
    for (int i = 0; i < ids.size(); i++) // id with label
        switch (mode) {
            case 0:
                outfile << ids[i] << "\t" << outpathname << "/" << ids[i] << "/classification.txt" << endl;
                break;
            case 1:
                outfile << ids[i] << "\t" << outpathname << "/" << ids[i] << "/functions.txt" << endl;
                break;
            case 2:
                outfile << ids[i] << endl;
                break;
            case 3:
                outfile << ids[i] << "\t" << outpathname << "/" << ids[i] << "/classification.rare.txt" << endl;
                break;
            default:
                break;
        }
