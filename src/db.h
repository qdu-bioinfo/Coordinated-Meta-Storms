...
unsigned int _PMDB::Read_Taxonomy(hash_map <string, string, std_string_hash> &table) {
    ...
    if (!infile) {
        cerr << "Error: Open Taxonomy Annotation file error : " << path << endl;
        exit(0);
    }
...
int _PMDB::Load_Copy_Number(hash_map<string, float, std_string_hash> &cp_number) {
    ...
    string cp_path = DB_cp_number;
    cp_path.erase(remove_if(cp_path.begin(), cp_path.end(), [](char c) {
        return c == '\r' || c == '\n' || c == '\t' || c == ' ';
    }), cp_path.end());

    ifstream in_cp_number(cp_path.c_str(), ifstream::in);
    if (!in_cp_number) {
        cerr << "Error: Cannot open file: [" << cp_path << "]" << endl;
        return 0;
    }
...
int _PMDB::Load_config() {
    ...
    config_path.erase(remove_if(config_path.begin(), config_path.end(), [](char c) {
        return c == '\r' || c == '\n' || c == '\t' || c == ' ';
    }), config_path.end());

    ifstream in_config(config_path.c_str(), ios::in);
    if (!in_config) {
        cerr << "Error: Cannot open reference database configuration: [" << config_path << "]" << std::endl;
        return 0;
    }
