...
public:
    ...
    float Calc_sim(float *Abd_1, float *Abd_2, bool w);//加权计算
    ...
    /*new code*/
    vector<float> getDist_1() {
        return Dist_1;
    }

    vector<float> getDist_2() {
        return Dist_2;
    }

    vector<int> getOrder_1() {
        return Order_1;
    }

    vector<int> getOrder_2() {
        return Order_2;
    }

    vector<int> getOrder_d() {
        return Order_d;
    }
...
private:
    _PMDB Database;//数据库对象

    vector<float> Dist_1;//距离
    vector<float> Dist_2;

    vector<int> Order_1;//顺序
    vector<int> Order_2;
    vector<int> Order_d;
    vector<string> Id;
...
void _Comp_Tree::Init() {//初始化_comp_tree类的变量

    LeafN = Load_id();//加载叶子节点的id
    // load id hash
    ...
    OrderN = 0;
    // load tree
    if (Database.Get_Is_Tree())//加载树的节点顺序信息
        OrderN = Load_order();

    // load cp number
    if (Database.Get_Is_Cp())//检查是否存在复制数信息
...
int _Comp_Tree::Load_abd(const char *infilename, float *Abd, bool is_cp_correct) {
    ...
    for (hash_map<string, int, std_string_hash>::iterator miter = otu_count.begin();
         miter != otu_count.end(); miter++) {
...
float _Comp_Tree::Calc_sim(float *Abd_1, float *Abd_2, bool w) {
    ...
    if (!Database.Get_Is_Tree())
        return 1.0 - Calc_Dist_JSD(Abd_1, Abd_2, LeafN);

    if (w)
        return Calc_sim(Abd_1, Abd_2);
    else
        return Calc_sim_unweight(Abd_1, Abd_2);
}

float _Comp_Tree::Calc_sim(float *Abd_1, float *Abd_2,
                           int mode) { // 0: MS; 1: Unweighted MS; 2: Cos; 3: Eu; 4: JSD; 5: Bray Curtis

    if (!Database.Get_Is_Tree()) {
        if (mode < 2)
            mode = 4; // for no tree, default is 4: JSD
    }
    float total = 0;
    switch (mode) {
        case 0:
            return Calc_sim(Abd_1, Abd_2);
//            return Calc_sim_GPU(Abd_1, Abd_2);
//            return Calc_sim_GPU(Abd_1, Abd_2, Load_order(), Order_1, Order_2, Order_d, Dist_1, Dist_2, total);
            break;
        case 1:
            return Calc_sim_unweight(Abd_1, Abd_2);
            break;
        case 2:
            return 1.0 - Calc_Dist_Cos(Abd_1, Abd_2, LeafN);
            break;
        case 3:
            return 1.0 - Calc_Dist_E(Abd_1, Abd_2, LeafN);
            break;
        case 4:
            return 1.0 - Calc_Dist_JSD(Abd_1, Abd_2, LeafN);
            break;
        case 5:
            return 1.0 - Calc_Dist_Bray_Curtis(Abd_1, Abd_2, LeafN);
            break;
        default:
            return 1.0 - Calc_Dist_JSD(Abd_1, Abd_2, LeafN);
            break;
